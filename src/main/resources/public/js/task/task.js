layui.use(['table', 'layer'], function () {
    ...
    var tableIns = table.render({
        ...
        cols: [[
            ...
            {title: '操作', templet: '#taskListBar', fixed: "right", align: "center", minWidth: 150}
        ]]
    });
    ...
    function formatterDevResult(value) {
        ...
        if (value == 0) {
            ...
            return "<div style='color: #00FF00;'>执行中</div>";
        } else if (value == 2) {
    ...
    table.on("tool(saleChances)", function (obj) {
        ...
        if (layEvent === "edit") {
            ...
            layer.confirm('确定删除当前数据？', {icon: 3, title: "任务管理"}, function (index) {
                layer.close(index);
                $.ajax({
                    type: "post",
                    url: ctx + "/task/delete",
    ...
    function openAddOrUpdateSaleChanceDialog(sid) {
        console.log(sid);
        var url = ctx + "/task/addOrUpdatePage";
        var title = "任务管理-任务添加";
        if (sid) {
            url = url + "?id=" + sid;
            console.log(url);
